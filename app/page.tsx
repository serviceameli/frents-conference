...
function Logo({ light = false }: { light?: boolean }) {
  return <a href="#" aria-label="Frents — на главную" className={`brand ${light ? "brand-light" : ""}`}><img src="/assets/frents-logo.svg" alt="Frents" width="110" height="36" /></a>;
}
...
function ProductPreview({ service, intent }: { service: ServiceId; intent: string }) {
  return <div className={`product-preview preview-${service}`}>
    ...
    {service === "rental" && <div className="app-window rental-window">
      ...
      <div className="app-body"><div className="app-sidebar"><Boxes size={20}/><FolderOpen size={20}/><CalendarDays size={20}/><UsersRound size={20}/></div><div className="app-main"><div className="app-title"><h4>Заказы</h4><span className="mini-button">+ Новый заказ</span></div><div className="app-stat-row"><div><small>В работе</small><b>12 <span>заказов</span></b></div><div><small>Ближайшая выдача</small><b>18 <span>сентября</span></b></div></div><div className="order-row"><span className="order-icon"><PackageCheck size={20}/></span><div><strong>Камерная свадьба</strong><small>18 сентября · 24 предмета</small></div><span className="status">Подтверждён</span></div><div className="order-row"><span className="order-icon"><Boxes size={20}/></span><div><strong>Ужин в саду</strong><small>21 сентября · 36 предметов</small></div><span className="status neutral">Новый</span></div><div className="catalog-strip"><img src="/assets/hero-tablescape.webp" alt="Коллекция декора с хромированными вазами" loading="lazy"/><div><strong>Ваша коллекция. Ваш бренд.</strong><small>Личный сайт + общий каталог Frents</small></div><ArrowUpRight size={18}/></div></div></div>
    </div>}
    {service === "projects" && <div className="app-window project-window"><div className="app-bar"><span className="mini-brand">FRENTS <span>workspace</span></span><span className="private-label"><LockKeyhole size={12}/> Только для вас</span></div><div className="project-cover"><img src="/assets/hero-tablescape.webp" alt="Флористика и сервировка для камерной свадьбы" loading="lazy"/><span>Мой проект</span></div><div className="project-content"><div className="app-title"><h4>Камерная свадьба</h4><span className="tiny-tag">18 сентября</span></div><p>Реквизит для монтажа</p>{[["Ваза хромированная", "6 шт.", true],["Подсвечник высокий", "12 шт.", true],["Текстиль лайм", "4 шт.", false]].map(([name, count, done])=><div className="packing-row" key={String(name)}><span className={done ? "packing-check done" : "packing-check"}>{done && <Check size={12}/>}</span><span>{name}</span><small>{count}</small><span className={`status ${!done ? "neutral" : ""}`}>{done ? "Свой склад" : "Найти"}</span></div>)}</div></div>}
    {service === "market" && <div className="app-window market-window"><div className="app-bar"><span className="mini-brand">FRENTS <span>барахолка</span></span><span className="tiny-tag green">Бесплатные объявления</span></div><div className="market-content"><div className="mock-search"><Search size={16}/> Декор для следующей идеи</div><div className="market-item"><div className="market-photo"><img src="/assets/hero-tablescape.webp" alt="Набор хромированных предметов декора" loading="lazy"/><span><Heart size={17}/></span></div><div className="market-item-info"><div><small>Из коллекции декоратора</small><h4>Хром. Новая история.</h4><p>Вазы, подсвечники и детали сервировки</p></div><span className="market-free">0 ₽<small>за размещение</small></span></div></div><div className="market-note"><Leaf size={18}/>{intent === "buy" ? "У вашей идеи уже может быть готовое решение" : "Ваш прошлый проект — чья-то новая идея"}</div></div></div>}
    {service === "specialists" && <div className="app-window specialist-window"><div className="app-bar"><span className="mini-brand">FRENTS <span>специалисты</span></span><span className="tiny-tag">Знакомьтесь по работам</span></div><div className="specialist-content"><div className="profile-top"><div className="profile-avatar"><img src="/assets/hero-tablescape.webp" alt="Флористическое портфолио" loading="lazy"/></div><div><small>Флористика и декор</small><h4>Студия «Форма»</h4><p><MapPin size={12}/> Москва</p></div></div><p className="profile-about">Создаём атмосферу события через цветы, фактуры и внимание к деталям.</p><div className="portfolio-title"><strong>Портфолио</strong><span>Услуги и цены <ArrowUpRight size={13}/></span></div><img className="portfolio-image" src="/assets/hero-tablescape.webp" alt="Пример работы: современная сервировка с белыми каллами" loading="lazy"/><div className="profile-footer"><span>Оформление событий</span><span className="mini-button">Написать <ArrowUpRight size={12}/></span></div></div></div>}
