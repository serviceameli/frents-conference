...
import { ArrowUpRight, ArrowRight, ArrowDown, Boxes, FolderOpen, Repeat2, UsersRound, Check, Gift, Plus, Smartphone, PackageCheck, CalendarDays, MapPin, Download, LoaderCircle, Sparkles, X, LockKeyhole, Warehouse, Store, Clock3 } from "lucide-react";
...
import { services, serviceIds, type ServiceId } from "@/lib/services";

const icons = { rental: Boxes, projects: FolderOpen, inventory: Warehouse, market: Repeat2, marketplace: Store, specialists: UsersRound };
const ids: ServiceId[] = [...serviceIds];
const serviceLabels: Record<ServiceId, string> = {
  rental: "Прокат", projects: "Дизайн-проекты", inventory: "Складской учёт", market: "Барахолка", marketplace: "Маркет", specialists: "Специалисты",
};
...
function ProductPreview({ service }: { service: ServiceId }) {
  if (service === "inventory" || service === "marketplace") return <LaunchPreview service={service}/>;
...
export default function ConferenceLanding({ initialService = "rental" }: { initialService?: ServiceId }) {
  ...
  const waitlistOnly = selected.length > 0 && selected.every(id=>services[id].comingSoon);
  const submittedWaitlist = submittedServices.filter(id=>services[id].comingSoon);
  const downloads = submittedServices.filter(id=>services[id].download);
  ...
  useEffect(() => {
    document.title = active === "market" ? "Барахолка FRENTS — декор получает вторую жизнь" : active === "marketplace" ? "Маркет FRENTS — скоро запуск" : active === "inventory" ? "Складской учёт FRENTS — скоро запуск" : "Frents — больше творчества, меньше рутины";
  }, [active]);
  ...
  const questions = active === "market" ? marketQuestions : [["Можно пользоваться только одним сервисом?","Да. Выберите нужное направление: аренда, дизайн-проекты, складской учёт, барахолка, маркет или специалисты. Для сервисов в разработке можно оставить заявку на запуск."],["Я декоратор и ничего не сдаю в аренду. Мне подходит?","Да. «Дизайн-проекты» помогают готовить оформление событий. А «Складской учёт» разрабатывается для работы с остатками, выездами, возвратами и занятостью изделий. Сдавать вещи в аренду не обязательно."],["Когда откроются Маркет и складской учёт?","Оба сервиса сейчас в разработке. Оставьте заявку на интересующее направление: мы сообщим о запуске и условиях подключения. Точную дату пока не объявляем."],["Чем Маркет отличается от барахолки?","На барахолке коллеги продают декор и вещи из своих коллекций. Маркет мы создаём для магазинов и производителей — с материалами, тканями, цветами, расходниками, готовым декором и конструкциями для ивент-индустрии."],["Барахолка бесплатна только для участников?","Нет. До конца 2026 года размещение объявлений бесплатно для всех пользователей. Бонус участника — полезный гид по продаже и покупке декора."],["Что произойдёт после заявки?","По существующим направлениям после отправки доступны материалы. Если выбрали Маркет или складской учёт, сохраним ваш интерес и сообщим о запуске. Свяжемся по указанному контакту."],["Сколько стоят сервисы?","До конца 2026 года размещение на барахолке бесплатно. Для студий аренды есть разные тарифы: подберём вариант под ваши задачи. Условия новых направлений обсудим при знакомстве."]];
  const conferenceSection = active === "market" ? <section className="wrap market-conference" id="conference"><div className="detail-bonus"><span className="bonus-small-icon"><Gift size={21}/></span><div><strong>На конференции — с приятным дополнением</strong><p>Гид по первому объявлению: фотографии, описание, цена и вопросы покупателю.</p></div><button className="text-link" onClick={()=>openForm("market")}>Забрать гид <ArrowRight size={17}/></button></div></section> : (<section className="conference-section wrap" id="conference"><div className="conference-panel"><div className="conference-copy"><span className="eyebrow"><Gift size={15}/> ДЛЯ УЧАСТНИКОВ КОНФЕРЕНЦИИ</span><h2>Ваш бонус<br/>для хорошего старта.</h2><p>Полезные материалы и особые условия для участников конференции. Выберите одно или несколько направлений.</p><button className="button button-dark" onClick={()=>openForm()}>Забрать свой бонус <ArrowUpRight size={18}/></button><span className="conference-micro">Одна короткая форма. Можно выбрать несколько сервисов.</span></div><div className="conference-ticket"><div className="ticket-top"><span className="mini-brand">FRENTS</span><span>ДЛЯ СВОИХ</span></div><div className="ticket-main"><span className="ticket-label">ВАШЕ ПРИГЛАШЕНИЕ</span><h3>Больше<br/>возможностей.</h3><div className="ticket-benefits"><p><Check size={15}/> Личный разбор задач проката</p><p><Check size={15}/> Ранний доступ к дизайн-проектам</p><p><Check size={15}/> Запуск складского учёта</p><p><Check size={15}/> Старт Маркета для поставщиков</p><p><Check size={15}/> Гид по продаже и покупке декора</p><p><Check size={15}/> Чек-лист для команды и портфолио</p></div></div><div className="ticket-bottom"><span>СОЗДАВАТЬ ЛЕГЧЕ ВМЕСТЕ</span><ArrowUpRight size={29}/></div></div></div></section>);
  const faqSection = (<section className="faq-section wrap" id="faq"><div><span className="eyebrow">ЕЩЁ ПАРА ДЕТАЛЕЙ</span><h2>{active === "market" ? "Ещё вопросы?" : <>Вы могли<br/> об этом подумать.</>}</h2></div><Accordion type="single" collapsible className="faq-list">{questions.map(([q,a],i)=><AccordionItem value={String(i)} key={q}><AccordionTrigger>{q}</AccordionTrigger><AccordionContent>{a}</AccordionContent></AccordionItem>)}</Accordion></section>);
  ...
  return <>
    ...
    <main>
      ...
      <section className={`services-section wrap ${active === "market" ? "market-service-select" : ""}`} id="services"><div className="section-heading"><div><div className="eyebrow">ПОД ВАШУ ЗАДАЧУ</div><h2>Чем Frents поможет вам?</h2></div><p>Выберите сервис — и сразу к делу.</p></div>
        <div className="service-tabs"><div className="service-grid" role="group" aria-label="Выберите задачу">{ids.map((id,index)=>{const s=services[id];const Icon=icons[id];return <button type="button" key={id} className={`service-card card-${id}`} data-state={active===id ? "active" : "inactive"} aria-pressed={active===id} onClick={()=>chooseService(id,true)}><span className="service-card-top"><span className="service-icon"><Icon size={23} strokeWidth={1.6}/></span><span className="service-number">0{index+1}</span></span><span className="service-audience">{s.audience}</span><span className="service-title">{s.cardTitle}</span>{s.comingSoon && <span className="card-launch-status"><Clock3 size={12}/> Скоро запуск</span>}<span className="service-summary">{s.cardDescription}</span><span className="service-card-bottom"><span>{s.tag}</span><span className="service-arrow"><ArrowUpRight size={20}/></span></span></button>})}</div>
        </div>
      </section>
      {active === "market" ? <div ref={detailRef} id="service-details" className="market-detail-start"><TabsContent value="market" className="market-service-content"><FleaLanding intent={intents.market} onIntentChange={value=>setIntents({...intents,market:value})} onOpenMarketplace={()=>chooseService("marketplace",true)} heroExtra={conferenceSection}><AppShowcase/>{faqSection}</FleaLanding></TabsContent></div> : <>
      <section className="service-details-section wrap"><div className="service-tabs">
          <div ref={detailRef} id="service-details" className="service-detail-start">
          {ids.filter(id=>id!=="market").map(id=><TabsContent value={id} key={id} className={`service-content ${services[id].comingSoon ? "launch-service-content" : ""}`}><div className="detail-grid"><div className="detail-copy"><div className="detail-label"><span>0{ids.indexOf(id)+1}</span> {services[id].name}</div>{id==="specialists"&&<Tabs value={intents[id]} onValueChange={v=>setIntents({...intents,[id]:v})} className="intent-tabs"><TabsList aria-label="Ваша задача"><TabsTrigger value="team">Ищу команду</TabsTrigger><TabsTrigger value="provider">Предлагаю услуги</TabsTrigger></TabsList></Tabs>}{services[id].comingSoon && <span className="launch-status"><Clock3 size={14}/> Скоро запуск</span>}<h3>{alternate && current.alternate ? current.alternate.title : services[id].title}</h3><p className="detail-description">{alternate && current.alternate ? current.alternate.description : services[id].description}</p><ul className="benefit-list">{benefits.map((b,i)=><li key={i}><span><Check size={15}/></span><div><strong>{b[0]}</strong><p>{b[1]}</p></div></li>)}</ul><div className="detail-actions"><button className="button button-dark" onClick={()=>openForm(id)}>{id==="specialists"&&intents.specialists==="provider" ? "Получить чек-лист профиля" : services[id].cta}<ArrowUpRight size={17}/></button><span>{services[id].footnote}</span></div></div><ProductPreview service={id}/></div><div className="detail-bonus"><span className="bonus-small-icon"><Gift size={21}/></span><div><strong>{services[id].comingSoon ? "Будьте среди первых" : "На конференции — с приятным дополнением"}</strong><p>{alternate && current.alternate ? current.alternate.bonus : services[id].bonus}</p></div><button className="text-link" onClick={()=>openForm(id)}>{services[id].comingSoon ? "Оставить заявку" : "Это для меня"} <ArrowRight size={17}/></button></div><Accordion type="single" collapsible className="service-extra" key={`extra-${id}`}><AccordionItem value="screens"><AccordionTrigger><span className="mobile-extra-label">{services[id].comingSoon ? "Подробнее о возможностях" : "Все возможности и ещё 3 экрана"}</span><span className="desktop-extra-label">{services[id].comingSoon ? "Как это будет работать" : "Ещё 3 экрана: как это работает"}</span></AccordionTrigger><AccordionContent><ul className="expanded-benefits">{benefits.map(([title,text])=><li key={title}><strong>{title}</strong><p>{text}</p></li>)}</ul><ScreenStoryboard service={id}/></AccordionContent></AccordionItem></Accordion></TabsContent>)}
          </div>
        </div>
      </section>
      <AppShowcase/><section className="ecosystem wrap"><span className="eyebrow">ОТ ЗАМЫСЛА ДО МОНТАЖА</span><h2>Ваши идеи. Ваши люди.<br/><span>Всё в одном кругу.</span></h2><div className="ecosystem-steps">{[{i:FolderOpen,t:"Соберите проект",d:"Начните с дизайн-проекта и своего реквизита."},{i:Boxes,t:"Найдите недостающее",d:"В аренде, на барахолке или в будущем Маркете."},{i:UsersRound,t:"Пригласите своих",d:"Специалистов, близких вам по стилю."},{i:Sparkles,t:"Создавайте",d:"Когда за кулисами всё в порядке."}].map((s,i)=><div key={i} className="ecosystem-step"><div><s.i size={22} strokeWidth={1.5}/><span>0{i+1}</span></div><h4>{s.t}</h4><p>{s.d}</p></div>)}</div></section>
      {conferenceSection}
      {faqSection}
      </>}
    </main>
    ...
    <div className="mobile-bonus"><span className="mobile-current-service"><small>0{ids.indexOf(active)+1} / {String(ids.length).padStart(2,"0")}</small><strong>{current.shortName}</strong></span><button onClick={()=>openForm(active)}>{current.comingSoon ? <>Оставить заявку <ArrowUpRight size={16}/></> : <>Забрать бонус <Gift size={16}/></>}</button></div>
    </Tabs>
    <Dialog open={formOpen} onOpenChange={v=>{if(!sending)setFormOpen(v)}}><DialogContent className="lead-dialog" showCloseButton={false}><DialogClose className="dialog-close" disabled={sending} aria-label="Закрыть форму"><X size={21}/></DialogClose>{sent ? <div className="success-state"><span className="success-icon"><Check size={29}/></span><span className="eyebrow">РАДЫ ЗНАКОМСТВУ</span><DialogTitle>{submittedWaitlist.length ? <>Вы в списке<br/>первых.</> : <>Всё начинается<br/>с хороших связей.</>}</DialogTitle><DialogDescription>{submittedWaitlist.length ? "Заявка получена. Сообщим о запуске и условиях подключения по указанному контакту." : "Заявка получена. Мы свяжемся с вами по указанному контакту."}</DialogDescription>{submittedWaitlist.length > 0 && <div className="waitlist-confirmation"><Clock3 size={19}/><div><strong>Ждём запуск вместе</strong><p>{submittedWaitlist.map(id=>services[id].shortName).join(" · ")}</p></div></div>}{downloads.length > 0 && <p className="download-intro">А материалы по выбранным направлениям уже здесь:</p>}<div className="download-list">{downloads.map(id=><a key={id} href={publicPath(`/bonuses/${id}.html`)} target="_blank" rel="noreferrer"><span>{services[id].download}</span><Download size={19}/></a>)}</div><button className="button button-dark" onClick={()=>setFormOpen(false)}>Продолжить знакомство <ArrowRight size={17}/></button></div> : <><span className="form-eyebrow">{waitlistOnly ? <><Clock3 size={18}/> РАННИЙ ДОСТУП</> : <><Gift size={18}/> БОНУСЫ И РАННИЙ ДОСТУП</>}</span><DialogTitle>{waitlistOnly ? <>Узнайте о запуске<br/>первыми.</> : <>Давайте создадим<br/>что-то хорошее.</>}</DialogTitle><DialogDescription>{waitlistOnly ? "Оставьте контакт — сообщим, когда выбранные сервисы будут готовы, и расскажем об условиях подключения." : "Отметьте интересные направления. Пришлём материалы, расскажем об условиях и запуске новых сервисов."}</DialogDescription><form onSubmit={submit} className="lead-form"><fieldset><legend>Что вам интересно?</legend><div className="form-services">{ids.map(id=><label className={`interest-chip ${selected.includes(id) ? "selected" : ""}`} key={id}><Checkbox checked={selected.includes(id)} onCheckedChange={checked=>setSelected(checked ? [...selected,id] : selected.filter(s=>s!==id))}/><span>{services[id].shortName}{services[id].comingSoon && <small>Скоро запуск</small>}</span></label>)}</div></fieldset><label className="field-label" htmlFor="lead-name">Как к вам обращаться <span>необязательно</span></label><input id="lead-name" name="name" autoComplete="name" maxLength={80} placeholder="Ваше имя" value={name} onChange={e=>setName(e.target.value)}/><label className="field-label" htmlFor="lead-contact">Telegram, телефон или email</label><input id="lead-contact" name="contact" autoComplete="off" required minLength={5} maxLength={120} placeholder="@username, +7… или email" value={contact} onChange={e=>setContact(e.target.value)}/><div className="honeypot" aria-hidden="true"><label>Ваш сайт<input name="website" tabIndex={-1} autoComplete="off"/></label></div><label className="consent-label"><Checkbox checked={consent} onCheckedChange={v=>setConsent(v===true)} aria-label="Согласие на обработку персональных данных"/><span>Даю <a href="https://frents.ru/moskva/personal-data-consent" target="_blank" rel="noreferrer">согласие на обработку персональных данных</a> для связи по заявке и принимаю <a href="https://frents.ru/moskva/policy-privacy" target="_blank" rel="noreferrer">политику конфиденциальности</a>.</span></label>{error&&<p className="form-error" role="alert">{error}</p>}<button type="submit" className="button button-dark submit-button" disabled={sending}>{sending ? <><LoaderCircle className="spin" size={18}/> Отправляем…</> : <>{waitlistOnly ? "Сообщить мне о запуске" : "Получить бонус и подробности"} <ArrowUpRight size={18}/></>}</button><p className="form-note">Свяжемся только по выбранным направлениям.</p></form></>}</DialogContent></Dialog>
  </>;
}
