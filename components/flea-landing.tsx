...
import { ArrowUpRight, Check, MapPin, Leaf, Repeat2, UsersRound, Sparkles, Globe2, Package, Search } from "lucide-react";
...
export default function FleaLanding({ children, heroExtra, intent, onIntentChange, onOpenMarketplace }: {
  children?: ReactNode;
 ...
 return <div className="flea-page flea-integrated">
   <section className="wrap service-detail-start fl-internal-hero" aria-labelledby="fl-internal-title">
    <div className="detail-grid">
     <div className="detail-copy">
      <div className="detail-label"><span>04</span> Барахолка Frents</div>
   ...
   {children}
   <section className="fl-container fl-shop-section"><div className="fl-shop"><div className="fl-shop-copy"><span className="fl-soon"><span/> Скоро на FRENTS</span><h2>Маркет<br/>для ивент-индустрии</h2><p>Готовим B2B-маркетплейс для магазинов и производителей: материалы, ткани, цветы, расходники, декор и конструкции.</p><button className="fl-button fl-primary" onClick={onOpenMarketplace}>Стать поставщиком Маркета <ArrowUpRight size={15}/></button></div><div className="fl-shop-art" aria-hidden="true"><div className="fl-shop-orbit"/><div className="fl-shop-card fl-shop-card-one"><span>НОВАЯ КОЛЛЕКЦИЯ</span><img src={publicPath("/assets/catalog-vase.png")} alt="" loading="lazy"/><div>Вазы и детали <ArrowUpRight size={15}/></div></div><div className="fl-shop-card fl-shop-card-two"><span>ОТ ПРОИЗВОДИТЕЛЯ</span><img src={publicPath("/assets/catalog-chair.png")} alt="" loading="lazy"/><div>Мебель для событий <ArrowUpRight size={15}/></div></div><span className="fl-shop-floating"><Sparkles size={15}/> Новое. Ваше.</span></div></div></section>
